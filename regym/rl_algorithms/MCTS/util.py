...
def random_selection_policy(obs,
    ...
    '''
    Gives a uniform distribution over :param: action_dim, as filtered by
    :param: legal_actions. Ignores all other parameters.
    '''
    if legal_actions == []: return []
    num_legal_actions = len(legal_actions) if legal_actions else action_dim
    action_probability = 1 / num_legal_actions
    return np.array([action_probability if a_i in legal_actions else 0.
            for a_i in range(num_legal_actions)])
