...
def test_can_run_multiple_async_episodes_of_multiagent_sequential_task(env_name):
    run_parallel_sequential_task_with_random_agent(env_name, EnvType.MULTIAGENT_SEQUENTIAL_ACTION,
                                        num_envs=4, num_episodes=2)
...
# This is duplication is needed as long as the Trajectory class isn't used everywhere
def run_parallel_sequential_task_with_random_agent(env_name, env_type,
                                        num_envs, num_episodes,
                                        model_based_agents=False):
    task = generate_task(env_name, env_type)
    # Random agens, either MCTS or random
