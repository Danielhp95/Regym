...
def self_play_training(env, training_agent, self_play_scheme, menagerie, target_episodes=10, opci=1, iteration=None):
    '''
    ...
    :param self_play_scheme: self-play scheme to be used, consisting of the following methods:
        opponent_sampling_distribution: Probability distribution that
        curator: Gating function which determines if the current agent will be added to the menagerie at the end of an episode
    :param menagerie: Menagerie that stores the policices.
    :param target_episodes: number of episodes that will be run before training ends.
    :param opci: Opponent policy Change Interval
    :param iteration: int, number of iteration that have been run already.
    ...
    for episode in range(target_episodes):
        ...
        menagerie = self_play_scheme.curator(menagerie, training_agent, episode_trajectory, iteration=iteration+episode)
